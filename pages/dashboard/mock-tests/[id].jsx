    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      <div className="bg-white border-b border-slate-100 px-8 py-4 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-heading">{mock?.title}</h1>
            <p className="text-xs text-muted font-bold uppercase tracking-wider">
              {mock?.level} Level Builder
            </p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-1 2xl:flex-row flex-col overflow-hidden p-6 gap-3 2xl:gap-6">
        {/* 1. SECTIONS SIDEBAR */}
        <aside className="w-full 2xl:w-1/4 bg-white rounded-[2.5rem] border border-slate-100 flex flex-col overflow-hidden shadow-sm">
          <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            <span className="font-black text-slate-800 uppercase text-sm tracking-widest">
              Bo'limlar
            </span>
            <button
              onClick={() => openModal("SECTION_FORM", { mockId }, "middle")}
          ...
          {activeSection ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-10">
              ...
              <p className="text-muted max-w-xs">
                Savollar va guruhlarni ko'rish uchun chap tomondan kerakli
                bo'limni tanlang
              </p>
            </div>
          )}
