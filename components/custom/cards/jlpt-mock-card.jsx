...
const MockCard = ({ item }) => {
  ...
  // Server rejects unpublishing while open exam assignments exist; mirror that
  // here so the user can't even click the button. Publish (DRAFT -> PUBLISHED)
  // remains allowed regardless.
  ...
  const handleTogglePublish = () => {
    ...
    openModal(
      "CONFIRM_MODAL",
      {
        ...
        mutateKey: [`mock-tests/`, router.locale],
        onConfirm: async () => {
          //   const action = isPublished ? "unpublish" : "publish";
          return await authAxios.post(`/mock-tests/${item.id}/publish/`);
        },
      },
      "small",
    );
  ...
  const handleClone = () => {
    openModal(
      "CONFIRM_MODAL",
      {
        ...
        mutateKey: [`mock-tests/`, router.locale],
        onConfirm: async () => {
          return await authAxios.post(`/mock-tests/${item.id}/clone/`);
        },
      },
      "small",
    );
  ...
  const handleDelete = () => {
    openModal(
      "CONFIRM_MODAL",
      {
        ...
        mutateKey: [`mock-tests/`, router.locale],
        onConfirm: async () => {
          return await authAxios.delete(`/mock-tests/${item.id}/`);
        },
      },
      "small",
    );
  ...
  const makedHref = `/dashboard/mock-tests/jlpt/${item.id}`;
    ...
    <div className="group bg-white border border-slate-100 p-5 rounded-[2.25rem] shadow-sm hover:shadow-2xl hover:shadow-orange-100/50 transition-all duration-500 flex flex-col h-full relative overflow-hidden text-left">
      ...
      <div className="absolute top-4 right-4 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition-all transform translate-y-[-10px] group-hover:translate-y-0">
        ...
        {!isPublished && isCreator && (
          <>
            <button
              onClick={() => openModal("MOCK_FORM", { mock: item }, "middle")}
