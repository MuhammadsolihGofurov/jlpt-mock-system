...
const JFTSubmissionLists = ({ customLoading }) => {
    const router = useRouter();
    const intl = useIntl();
    const { id: examId, page = 1 } = router.query;

    const { data, isLoading } = useSWR(
        examId ? [`jft-submissions/`, examId, page] : null,
        (url, id, p) =>
            fetcher(
                `${url}?jft_exam_assignment_id=${id}&page=${p}&page_size=8`,
                {},
                {},
                true,
            ),
    ...
    const exportToExcel = () => {
        const tableData = data.results.map((sub, index) => {
            ...
            let statusText = intl.formatMessage({ id: sub.results?.jft_result?.passed ? "Pass" : "Fail" });
            if (isDisqualified) statusText = intl.formatMessage({ id: "Disqualified" });

            return {
                "№": index + 1,
                [intl.formatMessage({ id: "STUDENT" })]: sub.student_display,
                [intl.formatMessage({ id: "Conversation & Expression" })]: Object.values(sub.results?.sections || {}).find(s => s.section_type === "CONVERSATION_EXPRESSION")?.score || 0,
                [intl.formatMessage({ id: "Listening" })]: Object.values(sub.results?.sections || {}).find(s => s.section_type === "LISTENING")?.score || 0,
                [intl.formatMessage({ id: "Vocabulary" })]: Object.values(sub.results?.sections || {}).find(s => s.section_type === "SCRIPT_VOCAB")?.score || 0,
                [intl.formatMessage({ id: "Reading" })]: Object.values(sub.results?.sections || {}).find(s => s.section_type === "READING")?.score || 0,
    ...
    const exportToPDF = () => {
        ...
        const tableRows = data.results.map((sub, index) => {
            ...
            let statusLabel = intl.formatMessage({ id: sub.results?.jft_result?.passed ? "Pass" : "Fail" });
            if (isDisqualified) statusLabel = intl.formatMessage({ id: "Disqualified" });

            return [
                (page - 1) * 8 + (index + 1),
                sub.student_display,
                Object.values(sub.results?.sections || {}).find(s => s.section_type === "CONVERSATION_EXPRESSION")?.score || 0,
                Object.values(sub.results?.sections || {}).find(s => s.section_type === "LISTENING")?.score || 0,
                Object.values(sub.results?.sections || {}).find(s => s.section_type === "SCRIPT_VOCAB")?.score || 0,
                Object.values(sub.results?.sections || {}).find(s => s.section_type === "READING")?.score || 0,
        ...
        autoTable(doc, {
            head: [[
                ...
                intl.formatMessage({ id: "Conversation & Expression" }),
                    ...
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {data.results.map((sub, index) => {
                                const sections =
                                    sub.results?.sections || {};


                                const isPassed = sub.results?.jft_result?.passed;
                                ...
                                const dateTime = formatDate(sub.completed_at)
...
export default JFTSubmissionLists;
