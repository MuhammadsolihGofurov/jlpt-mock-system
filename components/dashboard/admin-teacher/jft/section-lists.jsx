...
const SectionList = ({ mockId, activeSection, sections, onSelect, currentMockType }) => {
  ...
  const handleDeleteSection = (e, section) => {
    ...
    openModal(
      "CONFIRM_MODAL",
      {
        ...
        variant: "danger",
        onConfirm: async () => {
          await authAxios.delete(`${currentMockType?.section}${section.id}/`);
  ...
  const handleEditSection = (e, section) => {
    e.stopPropagation();
    openModal("SECTION_FORM", { mockId, section, currentMockType }, "middle");
  };
