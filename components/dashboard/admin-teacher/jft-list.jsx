...
import { JftCard, MockCard } from "@/components/custom/cards";
import { MockListsSkeleton } from "@/components/skeleton";

const JFTlist = ({customLoading}) => {
    const router = useRouter();
    const { modalClosed } = useModal();
    const { page = 1, search } = router.query;

    const { data, isLoading, mutate } = useSWR(
        [`jft-mock-tests/`, router.locale, page, search],
        (url, locale, p, q) => {
            ...
            {data?.results?.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {data.results.map((mock) => (
                        <JftCard key={mock.id} item={mock} mutate={mutate} />
                    ))}
                </div>
            ) : (
                <EmptyMessage
