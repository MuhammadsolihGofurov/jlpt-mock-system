...
export default function MyResultExam({ customLoading, examId }) {
    const intl = useIntl();
    ...
    const downloadPDF = async () => {
        ...
        doc.setFontSize(18);
        doc.text(intl.formatMessage({ id: "jlpt Score Report" }), 15, 34);
        ...
        drawRow(125, intl.formatMessage({ id: "Holat" }), intl.formatMessage({ id: jlpt?.passed ? "Pass" : "Fail" }));

        // 1. Avval body massivini tayyorlab olamiz
        const tableBody = [
            [
                intl.formatMessage({ id: 'Language Knowledge' }),
                jlpt?.section_results?.language_knowledge?.score || jlpt?.section_results?.language_reading_combined?.score || 0,
                {
                    content: `${Math.round(res.score)} / 180`,
                    rowSpan: 0,
                    styles: { valign: 'middle', halign: 'center', fontSize: 16, fontStyle: 'bold' }
                }
            ],
            jlpt?.section_results?.reading ? [
                intl.formatMessage({ id: 'Reading' }),
                jlpt?.section_results?.reading?.score || 0
            ] : null,
            [
                intl.formatMessage({ id: 'Listening' }),
                jlpt?.section_results?.listening?.score || 0
            ]
        ].filter(Boolean);

        if (tableBody.length > 0) {
            tableBody[0][2].rowSpan = tableBody.length;
        }
        ...
        autoTable(doc, {
            ...
            theme: 'grid',
            head: [[
                intl.formatMessage({ id: "Sections" }),
                intl.formatMessage({ id: 'Section Score' }),
        ...
        autoTable(doc, {
            ...
            theme: 'grid',
            head: [[intl.formatMessage({ id: 'Vocabulary' }), intl.formatMessage({ id: 'Grammar&Reading' }), intl.formatMessage({ id: 'Listening' })]],
            body: [[
